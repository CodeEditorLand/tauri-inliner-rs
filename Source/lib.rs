...
pub enum Error {
	/// A std::io::ErrorKind::NotFound error with the offending line in the
	/// string parameter
	#[error("`{0}`")]
	InvalidPath(String),
...
impl Default for Config {
	/// Enables everything
	fn default() -> Config {
		Config { inline_fonts:true, inline_remote:true, max_inline_size:5000 }
	}
}
...
/// Returns a `Result<String>` of the html file at file path with all the assets
/// inlined.
///
/// ## Arguments
...
/// Returns a `Result<String>` with all the assets linked in the the html string
/// inlined.
///
/// ## Arguments
/// * `html` - The html string.
/// * `root_path` - The root all relative paths in the html will be evaluated
